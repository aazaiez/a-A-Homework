...
#
# ```ruby
# a = { |num1, num2| num1 <=> num2 } #sort ascending
...
def substrings(string)
  ...
  0.upto(string.length-1) do |idx1|
    idx1.upto(string.length-1) do |idx2|
      word = ""
      word = string[idx1..idx2]
      new_arr << word if subwords(word, ["cat", "a", "at"])
    end
  end
...
def subwords(word, dictionary)
  dictionary.include?(word) ? true : false
end
...
class Array
  ...
  def my_select(&prc)
    def my_each(&prc)
      ...
      0.upto(self.length-1) do |idx1|
        new_arr << self[idx1] if prc.call(self[idx1])
      end
